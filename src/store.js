...
export default new Vuex.Store({
  ...
  actions: {
    signUp ({commit, dispatch}, authData) {
      axiosAuth
        // Web API key
        // AIzaSyCbBWiAd9VqJMg5vS2xWHQ294i1W8_rCmc
        // https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=[API_KEY]
        .post('accounts:signUp?key=AIzaSyCbBWiAd9VqJMg5vS2xWHQ294i1W8_rCmc',
          {
    ...
    signIn ({ commit, dispatch }, authData) {
      axiosAuth
        .post(
          'accounts:signInWithPassword?key=AIzaSyCbBWiAd9VqJMg5vS2xWHQ294i1W8_rCmc',
          {
            email: authData.email,
            password: authData.password,
            returnSecureToken: true
    ...
    storeUser ({state}, userData) {
      ...
      globalAxios
        .post('https://lin00146-mtm6430-final.firebaseio.com/users.json' +
              '?auth=' +
              state.idToken,
        userData
        )
    ...
    fetchUser ({commit, state}, userEmail) {
      ...
      globalAxios
        .get('https://lin00146-mtm6430-final.firebaseio.com/users.json' +
            '?auth=' +
            state.idToken
        )
        .then(res => {
    ...
    updateUser ({state}) {
      ...
      globalAxios
        .patch('https://lin00146-mtm6430-final.firebaseio.com/users/' +
            state.user.id +
            '.json' +
            '?auth=' +
            state.idToken,
